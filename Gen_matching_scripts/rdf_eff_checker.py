...
file_input = "/eos/user/j/jodedra/AnalysisWork_2024/CHECKING_EVERY_STEP_28_10_25/unbiased_fonll_filter_check/CMSSW_12_4_11_patch3/src/BtoKJpsiEE.root"
tree = "DecayTree"

df = ROOT.RDataFrame(tree, file_input)
...
df = df.Define("e1_pt_1", "e1_pt[0]") \
       .Define("e2_pt_1", "e2_pt[0]") \
       .Define("e1_eta_1", "e1_eta[0]") \
       .Define("e2_eta_1", "e2_eta[0]") \
       .Define("K_pt_1", "K_pt[0]") \
       .Define("K_eta_1", "K_eta[0]") \
...
# Take *first* B candidate weight per event


# Selection
cut = (
    "e1_pt_1>3.0 && e2_pt_1>3.0 && "
    "abs(e1_eta_1)<1.5 && abs(e2_eta_1)<1.5 && "
    "K_pt_1>0.5 && abs(K_eta_1)<2.5"
)
...
den_no = df.Count().GetValue()
num_no = df.Filter(cut).Count().GetValue()
print("Total events:", den_no)
print("Selected events:", num_no)

den_w = df.Sum("FONLL_weight").GetValue()
num_w = df.Filter(cut).Sum("FONLL_weight").GetValue()

print("Total weighted events:", den_w)
print("Selected weighted events:", num_w)

eff_no = num_no / den_no
eff_w = num_w / den_w

print("Efficiency without FONLL weights:", eff_no)
print("Efficiency with FONLL weights:", eff_w)